...
public sealed class GetMangaHandler : IRequestHandler<GetMangaQuery, GetMangaResponse>
{
    ...
    private IEnumerable<ChapterDto> MapAndReturnChapters(Manga manga)
    {
        ...
        if (!_currentUserAccessor.IsLoggedIn) return manga.Chapters.Select(x => new ChapterDto(x.Id, x.SourceId, x.Source.Name, x.Date, x.Number));

        var userMangaInfo = manga.UserMangas
            .SelectMany(um => um.UserChapters, (um, uc) => new UserMangaDto(um.MangaId, uc.SourceId, uc.ChapterId, uc.Chapter?.Number))
            .ToList();
